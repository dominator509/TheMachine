...
function fail(label, msg) { console.log('  \x1b[31m✗\x1b[0m', label + ':', msg); failed++; }
...
setTimeout(async () => {
  ...
  await test('POST /api/save-theme creates theme file', async () => {
    const testTheme = {
      ...
      stations: { 'smoke-station': { name: 'Smoke', gridCols: 4, gridRows: 2, tileSize: 32, slots: [{col:0,row:0}] } },
  ...
  try { const { unlinkSync } = await import('node:fs'); unlinkSync('/root/Machine/packages/service/src/gui/themes/smoke-test-theme.json'); } catch {}
  ...
  await test('POST /api/pipeline-event accepts valid event', async () => {
    const r = await fetch(BASE + '/api/pipeline-event', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ eventId: 's1', agentId: 3, eventType: 'status', station: 'coding', payload: { msg: 'compiling' } }),
    });
    const data = await r.json();
