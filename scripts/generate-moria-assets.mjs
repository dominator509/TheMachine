...
backdrops['testing-cavern'] = (w, h) => {
  ...
  bg += defs(torchGlowGradient());
  // Cracked floor
  bg += path(`M0,${h-28} L40,${h-30} L80,${h-26} L120,${h-32} L160,${h-28} L200,${h-30} L${w},${h-28}`, C.stoneLit, null, 0);
...
backdrops['durins-door'] = (w, h) => {
  ...
  bg += stoneWall(w, h);
  // The great door
  const dw = 56, dh = 72, dx = (w - dw) / 2, dy = (h - dh) / 2 + 4;
  bg += rect(dx - 4, dy - 4, dw + 8, dh + 8, C.stoneDark);
  bg += rect(dx, dy, dw, dh, C.door);
  bg += rect(dx + 2, dy + 2, dw - 4, dh - 4, C.doorEdge);
  // Door carvings
  bg += path(`M${dx+8},${dy+dh/2} Q${dx+dw/2},${dy+6} ${dx+dw-8},${dy+dh/2}`, C.stoneDark);
  bg += path(`M${dx+8},${dy+dh/2} Q${dx+dw/2},${dy+dh-6} ${dx+dw-8},${dy+dh/2}`, C.stoneDark);
  bg += circle(dx + dw / 2, dy + dh / 2, 6, C.mithril, 0.2);
  // Ithildin runes (faint)
  bg += path(`M${dx+12},${dy+14} Q${dx+dw/2},${dy+4} ${dx+dw-12},${dy+14}`, C.mithril, C.mithril, 1);
  // Watcher posts
...
backdrops['great-gates'] = (w, h) => {
  ...
  // Gates
  const gw = 52, gh = 64, gx = (w - gw) / 2, gy = 16;
  ...
  // Dwarven star above
  bg += path(`M${gx+gw/2},${gy-10} L${gx+gw/2+6},${gy-2} L${gx+gw/2+12},${gy+2} L${gx+gw/2+6},${gy+6} L${gx+gw/2+4},${gy+14} L${gx+gw/2},${gy+8} L${gx+gw/2-4},${gy+14} L${gx+gw/2-6},${gy+6} L${gx+gw/2-12},${gy+2} L${gx+gw/2-6},${gy-2} Z`, C.gold, 0.7);
  // Guard posts
...
backdrops['mithril-vein'] = (w, h) => {
  ...
  // Sparkle points
  for (const [sx, sy] of [[w*0.15, h*0.35], [w*0.45, h*0.28], [w*0.7, h*0.42], [w*0.35, h*0.65], [w*0.8, h*0.75]]) {
    bg += circle(sx, sy, 2, C.mithrilGlow, 0.8);
    bg += path(`M${sx},${sy-4} L${sx+1},${sy-1} L${sx+4},${sy} L${sx+1},${sy+1} L${sx},${sy+4} L${sx-1},${sy+1} L${sx-4},${sy} L${sx-1},${sy-1} Z`, C.mithrilGlow);
  }
  // Pickaxes leaning
...
backdrops['treasury'] = (w, h) => {
  ...
  // Gems scattered
  bg += polygon(`${w/2-20},${h-32} ${w/2-18},${h-36} ${w/2-16},${h-32} ${w/2-18},${h-30}`, C.gem);
  bg += polygon(`${w/2+10},${h-34} ${w/2+12},${h-38} ${w/2+14},${h-34} ${w/2+12},${h-32}`, C.gemBlue);
  bg += circle(w / 2 + 26, h - 33, 3, C.gemGreen);
  bg += polygon(`${w-54},${h-30} ${w-52},${h-34} ${w-50},${h-30} ${w-52},${h-28}`, C.gem);
  // Iron-bound door
...
function drawDwarf(id, frame, state) {
  ...
  if (helm === 'pointed') {
    ...
    parts += path(`M10,${10+bobY} Q16,${5+bobY} 22,${10+bobY} L20,${15+bobY} L12,${15+bobY} Z`, tunicC);
  } else {
    // Bare head / hair
    parts += path(`M10,${9+bobY} Q16,${6+bobY} 22,${9+bobY}`, t.beard === 'blonde' ? '#d4a050' : '#4a3020');
  }
...
async function main() {
  ...
  for (const [name, [w, h]] of Object.entries(stationDims)) {
    ...
    if (!genFn) { console.log(`  ⚠  no generator for "${name}", skipping`); continue; }
