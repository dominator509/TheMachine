...
(async () => {
  ...
  try {
    ...
    page.on('requestfailed', req => console.log('  REQ FAIL:', req.url().substring(0, 100), '—', req.failure()?.errorText?.substring(0, 50)));
    ...
    const state = await page.evaluate(() => {
      ...
      const panelInfo = Array.from(panels).slice(0, 3).map(p => {
      ...
      const spriteInfo = Array.from(sprites).slice(0, 5).map(s => {
