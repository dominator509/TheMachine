...
const C = {
  iron: '#555555', ironBright: '#888888', gold: '#d4a017',
  wood: '#6b4226', skin: '#deb887', skinShadow: '#c4a46a',
  stone: '#3a3530', stoneLit: '#5a5045', fire: '#e8601c',
  torch: '#e8943a', door: '#444444',
};

const BEARDS = { white:'#e8e0d0', grey:'#a0a0a0', brown:'#6b4226', red:'#b84020', blonde:'#d4a050', black:'#2a2a2a', silver:'#c0c0c8', ginger:'#d47030' };
const TUNICS = { brown:'#8B4513', green:'#2E5E2E', blue:'#2E3E6E', red:'#6E2E2E', purple:'#4E2E6E', grey:'#5E5E5E', gold:'#8E6E1E', teal:'#1E5E5E' };
...
function rect(x,y,w,h,f,rx){return`<rect x="${x}" y="${y}" width="${w}" height="${h}" fill="${f}"${rx!=null?` rx="${rx}"`:''}/>`;}
function circle(cx,cy,r,f){return`<circle cx="${cx}" cy="${cy}" r="${r}" fill="${f}"/>`;}
function ellipse(cx,cy,rx,ry,f){return`<ellipse cx="${cx}" cy="${cy}" rx="${rx}" ry="${ry}" fill="${f}"/>`;}
function polygon(pts,f){return`<polygon points="${pts}" fill="${f}"/>`;}
function path(d,f,s,sw){const st=s?` stroke="${s}" stroke-width="${sw||1}"`:'';return`<path d="${d}" fill="${f}"${st}/>`;}
function line(x1,y1,x2,y2,s,sw){return`<line x1="${x1}" y1="${y1}" x2="${x2}" y2="${y2}" stroke="${s}" stroke-width="${sw||1}"/>`;}
