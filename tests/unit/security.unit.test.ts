...
describe("redactSecret", () => {
  it("should mask middle of a long value", () => {
    const result = redactSecret("sk-abc12test3def456ghi789jkl");
...
describe("redactText", () => {
  it("should redact OpenAI-style API keys", () => {
    const text = "My key is sk-abc12test3def456ghi789jklmno and it's secret.";
    const result = redactText(text);
    expect(result.matchedPatterns).toContain("OPENAI_KEY");
    expect(result.redacted).toContain("[REDACTED_OPENAI_KEY(");
    expect(result.redacted).not.toContain("sk-abc12test3def456ghi789jklmno");
  });
  ...
  it("should detect multiple secret types in one text", () => {
    const text = `sk-abc12test3def456ghi789jklmnopqrstuvwxyz\nghp_abc123def456ghi789jklmnopqrs1234567890abcdef`;
    const result = redactText(text);
...
describe("containsSecret", () => {
  it("should return true for text with API keys", () => {
    expect(containsSecret("sk-abc12test3def456ghi789jklmno")).toBe(true);
  });
